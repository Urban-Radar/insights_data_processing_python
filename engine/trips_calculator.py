...
class TripsCalculator:
    ...
    def filter_data_relevant_to_perimeter(self, floating_car_data: DuckDBPyRelation,
    ...
    def extract_and_label_segments(self, vectors_in_area_of_interest: DuckDBPyRelation) -> DuckDBPyRelation:

        vectors_by_vehicle_and_time: DuckDBPyRelation = self.connection.sql(f"SELECT unique_id, {TIMESTAMP} as timestamp_start, "
    ...
    def extract_trips_with_stops(self, vectors_by_vehicle_and_time: DuckDBPyRelation):
         vectors_by_vehicle_and_time = self.connection.sql(f"SELECT * , "
         ...
         print(self.connection.sql(f"SELECT count(unique_id), count(time_delta), sum(is_stop), sum(is_new_stop) FROM vectors_by_vehicle_and_time WHERE is_stop = 1"))
